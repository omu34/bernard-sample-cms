        ...
        <div class="mx-auto grid max-w-2xl auto-rows-fr grid-cols-1 gap-8 sm:mt-4 lg:mx-0 lg:max-w-none lg:grid-cols-4">
            @foreach ($videos as $video)
                <article data-aos="fade-up" data-aos-duration="1500"
                    ...
                    <a href="{{ $video->link }}{{ $video->id }}" class="absolute inset-0 z-10">
                        <span class="sr-only">Go to {{ $video->title }}</span>
                    </a>
                    ...
                    @elseif (Str::startsWith($mimeType, 'image'))
                        <img src="{{ Storage::url($video->file_path) }}" alt=""
                    ...
                    @else
                        @php
                            ...
                            if (Str::startsWith($mimeType, 'application/pdf')) {
                                ...
                                $fileIcon = 'csv.png';
                            } elseif (Str::startsWith($mimeType, 'application/msword') || Str::startsWith($mimeType, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')) {
                                $fileIcon = 'word.png';
                            } elseif (Str::startsWith($mimeType, 'application/vnd.ms-excel') || Str::startsWith($mimeType, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')) {
                                $fileIcon = 'excel.png';
                            } elseif (Str::startsWith($mimeType, 'application/vnd.ms-powerpoint') || Str::startsWith($mimeType, 'application/vnd.openxmlformats-officedocument.presentationml.presentation')) {
                    ...
                    <div class="flex flex-wrap items-center gap-y-1 overflow-hidden text-sm leading-6 text-white">
                        <time datetime="{{ $video->created_at->format('Y-m-d') }}"
                            class="text-white mr-6">{{ $video->created_at->format('M d, Y') }}</time>
                    ...
                    <div class="mt-2">
                        <button wire:click=""
                            class="text-white bg-{{ $video->is_featured ? 'yellow' : '' }}-500 hover:bg-{{ $video->is_featured ? 'yellow' : '' }}-700 font-bold py-2 px-4 rounded">
